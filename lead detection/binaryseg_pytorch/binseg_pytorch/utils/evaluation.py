...
class Evaluation():
    ...
    def __call__(self, gtdir, resdir, resprefix):
        ...
        listImgFiles = [k.split('/')[-1].split('.')[0] for k in glob.glob(os.path.join(gtdir, 'val_*.tif'))]
        print(listImgFiles)
        for currFile in tqdm(listImgFiles):
            res = np.array(Image.open(os.path.join(resdir, currFile + '_' + resprefix + '.tif')).convert('L'))
            res = np.float32(res)/255

            gt = np.array(Image.open(os.path.join(gtdir, currFile + '.tif')).convert('L'))
    ...
    def specificity_sensitivity(self, gt, res):
        ...
        #  返回平均交并比mIoU
        intersection = np.diag(confusion_matrix(A, B))
        union = np.sum(confusion_matrix(A, B), axis=1) + np.sum(confusion_matrix(A, B), axis=0) - np.diag(confusion_matrix(A, B))
        IoU = intersection / union
        mIoU = np.nanmean(IoU)

        return f_score, mIoU, accuracy
